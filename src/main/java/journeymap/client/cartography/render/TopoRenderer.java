...
public class TopoRenderer extends BaseRenderer implements IChunkRenderer {
            ...
            for (int x = 0; x < 16; ++x) {
                ...
                for (int i = 0; i < heights.length; ++i) {
                    ...
                    if (isWater && !isShore) {
                        ...
                        final int newZ = (chunkMd.getCoord().z << 4) + (z + offset.z) & 0xF;
    ...
    public int getBlockHeight(final ChunkMD chunkMd, final BlockPos blockPos) {
        return Journeymap.clientWorld().getChunkFromBlockCoords(blockPos).getPrecipitationHeight(blockPos).getY();
    }
