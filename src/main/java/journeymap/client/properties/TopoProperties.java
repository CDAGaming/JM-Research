...
public class TopoProperties extends ClientPropertiesBase implements Comparable<TopoProperties> {
    ...
    static {
        HEADERS = new String[]{"// " + Constants.getString("jm.config.file_header_1"), "// " + Constants.getString("jm.config.file_header_5", "http://journeymap.info/Topographic")};
    }

    public final BooleanField showContour;
    public final StringField landContour;
    public final StringField waterContour;
    public final StringField land;
    public final StringField water;
    ...
    public TopoProperties() {
        this.showContour = new BooleanField(Category.Hidden, true);
        this.landContour = new StringField(Category.Hidden, "").set("#3F250B");
        this.waterContour = new StringField(Category.Hidden, "").set("#0000dd");
        this.land = new StringField(Category.Hidden, "").multiline(true).set("#010c02,#041105,#071609,#0a1b0c,#0d200f,#102513,#132a16,#162f19,#19341c,#1b3a20,#1e3f23,#214426,#24492a,#274e2d,#2a5330,#2d5834,#375f41,#41674d,#4b6e5a,#557567,#5f7c73,#698480,#738b8d,#7c929a,#8699a6,#90a1b3,#9aa8c0,#a4afcc,#aeb6d9,#b8bee6,#c2c5f2,#ccccff,#cccfff,#ccd2ff,#ccd6ff,#ccd9ff,#ccdcff,#ccdfff,#cce2ff,#cce5ff,#cce9ff,#ccecff,#ccefff,#ccf2ff,#ccf5ff,#ccf9ff,#ccfcff,#ccffff,#cfffff,#d2ffff,#d6ffff,#d9ffff,#dcffff,#dfffff,#e2ffff,#e5ffff,#e9ffff,#ecffff,#efffff,#f2ffff,#f5ffff,#f9ffff,#fcffff,#ffffff");
        this.water = new StringField(Category.Hidden, "").multiline(true).set("#000040,#02024e,#03035d,#05056b,#070779,#080887,#0a0a96,#0b0ba4,#1a1aaa,#2a2aaf,#3939b5,#4848bb,#5757c0,#6767c6,#7676cc,#8585d2,#9494d7,#a4a4dd,#b3b3e3,#c2c2e8,#d1d1ee,#d7d7f0,#ddddf2,#e2e2f4,#e8e8f6,#eeeef9,#f4f4fb,#f9f9ff,#f9f9ff,#f9f9ff,#f9f9ff,#f9f9ff");
    }
    ...
    private Integer[] getColors(final String name, final String colorString) {
        final String[] colors = colorString.split(",");
        final int size = Math.min(128, colors.length);
        if (size == 0) {
        ...
        final Integer[] colorInts = new Integer[size];
    ...
    public String[] getHeaders() {
        return TopoProperties.HEADERS;
    }

    private boolean isValid(final String colorString, final boolean fix) {
        boolean valid = true;
        final String[] colors = colorString.split(",");
        for (int i = 0; i < colors.length; ++i) {
